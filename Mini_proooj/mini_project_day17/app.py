...
def handle_application():    
    ...
    if action=='apply':
        ...
        if companies_applied[0] != None:
            ...
            companies_applied=(company_name)
        companies_applied=",".join(companies_applied)
        cur.execute("UPDATE student_details SET applied_companies = %s  WHERE userid = %s",(companies_applied,session['user_id']))
        mysql.connection.commit()
        cur.close()
        print(companies_applied)
        print(companies_applied)
    return "hi world"

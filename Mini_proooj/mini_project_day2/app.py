...
@app.route('/hr_dashboard/current_requirements')
def current_requirements():
    ...
    if temp is not None:
        return render_template('current_requirements.html', requirement1=temp[2], requirement2=temp[3])
    



@app.route('/hr_dashboard/elgible_students')
def eligible_students():
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM company_requirements WHERE company_name = %s" , (session['company_name'],))
    temp_company_requirements = cur.fetchone()

    cur.execute("SELECT * FROM student_details WHERE cgpa >= %s AND dob <= %s", (temp_company_requirements[2],temp_company_requirements[3]))

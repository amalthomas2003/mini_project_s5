...
def student_dashboard():
    ...
    if 'user_id' in session and session['role'] == 'student':
        ...
        cursor.execute("SELECT name,cgpa,dob,batch,points FROM student_details WHERE userid = %s", (session["user_id"],))
...
def post_questions():
    print("Hi")
    if request.method == 'POST':
        # Retrieve tags and content from the form
        print(request.form)
        print("HI")
        ...
        cur.execute("INSERT INTO questions(tags,userid,username,question,status,question_date) VALUES (%s,%s,%s,%s,%s,%s)",(tags,session['user_id'],session['student_name'],content,0,question_date))
        mysql.connection.commit()
        cur.close()
